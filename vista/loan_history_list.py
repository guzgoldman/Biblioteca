...
class LoanHistoryList(BaseApp):
    ...
    def load_data(self):
        ...
        devueltos = (
            ...
            .options(
                joinedload(Prestamo.socio),  # Carga socio completo
                joinedload(Prestamo.ejemplar).joinedload(Ejemplar.libro)  # Carga ejemplar + libro
            )
            .filter(Prestamo.fecha_devolucion != None)
            .all()
        )
